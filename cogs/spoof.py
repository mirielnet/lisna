...
class Spoof(commands.Cog):
    ...
    @app_commands.command(
        name="spoof",
        description="指定したユーザーまんまのWebHookを作成し、言わせたい言葉を送信します。",
    )
    @app_commands.describe(
        user="なりすまししたいユーザーを指定してください。",
        message="言わせたい言葉を入力してください。",
    )
    async def spoof(
        ...
        try:
            ...
            if webhook is None:
                created_webhook = await interaction.channel.create_webhook(
                    name=user.name, avatar=await user.display_avatar.read()
                )
            ...
            await interaction.followup.send(
                f"{user.name} のなりすましWebHookを作成し、指定した言葉を送信しました。",
                ephemeral=True,
            )
            ...
            await interaction.followup.send(
                "WebHookの作成やメッセージの送信中にエラーが発生しました。",
                ephemeral=True,
            )
