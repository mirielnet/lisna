...
class Vote(commands.Cog):
    ...
    async def register_existing_votes(self):
        ...
        votes = await db.execute_query(
            "SELECT message_id, channel_id, options, creator_id FROM votes"
        )
        ...
        for vote in votes:
            ...
            if channel is None:
                print(
                    f"Channel with ID {channel_id} not found. Skipping message ID {message_id}."
                )
                continue
            ...
            try:
                ...
                view = VoteView(
                    bot=self.bot, option_list=options, creator_id=creator_id
                )
                await message.edit(view=view)

            except discord.NotFound:
                print(
                    f"Message with ID {message_id} not found. Deleting from database."
                )
                await db.execute_query(
                    "DELETE FROM votes WHERE message_id = $1", (message_id,)
                )
                await db.execute_query(
                    "DELETE FROM vote_results WHERE message_id = $1", (message_id,)
                )
    ...
    async def create_vote(
        ...
        try:
            ...
            await interaction.response.send_message(
                "締め切り日時の形式が正しくありません。", ephemeral=True
            )
            return
        ...
        embed = discord.Embed(
            title=title,
            description="投票は1回限りです。選択してください。",
        ...
        embed.set_footer(
            text=f"投票締め切り時刻: {deadline_dt.strftime('%Y/%m/%d %H:%M')}"
        )

        view = VoteView(
            bot=self.bot, option_list=option_list, creator_id=interaction.user.id
        )
    ...
    async def check_votes(self):
        jst = datetime.timezone(datetime.timedelta(hours=9))
        now = datetime.datetime.now(jst).replace(
            tzinfo=None
        )  # タイムゾーンを削除して比較

        results = await db.execute_query(
            "SELECT message_id, channel_id, options FROM votes WHERE deadline <= $1",
            (now,),
        )
        ...
        for row in results:
            ...
            try:
                ...
                print(
                    f"Message with ID {message_id} not found in channel {channel_id}. Deleting from database."
                )
                await db.execute_query(
                    "DELETE FROM votes WHERE message_id = $1", (message_id,)
                )
    ...
    async def display_results(self, message, options):
        results = await db.execute_query(
            "SELECT option_index, COUNT(*) FROM vote_results WHERE message_id = $1 GROUP BY option_index",
            (message.id,),
        )
        if results is None:
            results = []
        ...
        for idx, option in enumerate(options):
            ...
            embed.add_field(
                name=option, value=f"{count}票 ({percentage:.2f}%)", inline=False
            )
...
class VoteView(View):
    ...
    @discord.ui.button(
        label="終了", style=discord.ButtonStyle.danger, custom_id="vote_end"
    )
    async def end_vote(
        ...
        if interaction.user.id != self.creator_id:
            await interaction.followup.send(
                "投票の終了は作成者のみが可能です。", ephemeral=True
            )
            return
        ...
        await db.execute_query(
            "DELETE FROM votes WHERE message_id = $1", (interaction.message.id,)
        )
        await db.execute_query(
            "DELETE FROM vote_results WHERE message_id = $1", (interaction.message.id,)
        )
