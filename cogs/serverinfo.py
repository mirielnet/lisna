...
class ServerInfo(commands.Cog):
    ...
    async def server_info(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="テキストチャンネル数", value=str(text_channels), inline=True
        )
        embed.add_field(
            name="ボイスチャンネル数", value=str(voice_channels), inline=True
        )
        embed.add_field(name="絵文字数", value=str(emoji_count), inline=True)
        embed.add_field(
            name="アニメーション絵文字数", value=str(animated_emoji_count), inline=True
        )
        ...
        # 絵文字のリスト
        emoji_list = "".join(
            [str(emoji) for emoji in emojis[:50]]
        )  # 最大50個の絵文字を表示
        if emoji_list:
