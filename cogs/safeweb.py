...
class SafeWeb(commands.Cog):
    ...
    async def safeweb(self, interaction: discord.Interaction, url: str):
        ...
        try:
            ...
            # Use regex to search for the specific status in the HTML response
            if re.search(r'<p[^>]*class="rating-label[^"]*">警告</p>', result):
                color = discord.Color.red()
                title = "このサイトは危険です"
                description = ("これは既知の危険な Web サイトです。"
                               "このページを表示**しない**ことを推奨します。")
            elif re.search(r'<p[^>]*class="rating-label[^"]*">未評価</p>', result):
                color = discord.Color.gray()
                title = "このサイトは評価されていません"
                description = ("サイトは未評価のため、接続には注意が必要な可能性があります。")
            elif re.search(r'<p[^>]*class="rating-label[^"]*">安全</p>', result):
