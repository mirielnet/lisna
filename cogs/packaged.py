    ...
    async def packaged(self, interaction: discord.Interaction, type: app_commands.Choice[str], name: str):
        ...
        try:
            if type.value == "npm":
                ...
                embed.add_field(name="作者", value=pkg.get('author', {}).get('name', 'なし'), inline=True)
                embed.add_field(name="バージョン", value=pkg.get('version', '不明'), inline=True)
                embed.add_field(name="リポジトリ", value=pkg['links'].get('repository', 'なし'), inline=True)
                ...
                embed.add_field(name="作者", value=pkg.get('author', 'なし'), inline=True)
                embed.add_field(name="バージョン", value=pkg.get('version', '不明'), inline=True)
                embed.add_field(name="リポジトリ", value=pkg.get('project_urls', {}).get('Home', 'なし'), inline=True)
                embed.add_field(name="ライセンス", value=pkg.get('license', 'なし'), inline=True)
                embed.add_field(name="キーワード", value=pkg.get('keywords', 'なし'), inline=True)
