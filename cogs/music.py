    ...
    async def play(
        ...
        if re.match(url_pattern, url) is not None:
            ...
            with youtube_dl.YoutubeDL(
                ...
                # 検索結果を5件取得
                videos = ytdl.extract_info(f"ytsearch5:{url}", download=False)[
                    "entries"
                ]
            # 配列から必要な情報を抽出
            entries = [
                (entry["title"], f"https://www.youtube.com/watch?v={entry['id']}")
                for entry in videos
            ]
            select = discord.ui.Select(
                placeholder="検索結果",
                options=[
                    discord.SelectOption(label=title, description=url, value=url)
