...
class HitAndBlowServer(commands.Cog):
    ...
    @app_commands.command(
        name="hitandblow-server",
        description="サーバー内でヒット＆ブローをプレイします。",
    )
    @app_commands.describe(digits="桁数（初期値: 4桁）")
    async def hitandblow_server(
        self, interaction: discord.Interaction, digits: int = 4
    ):
        if interaction.guild.id in self.sessions:
            await interaction.response.send_message(
                "既にゲームが進行中です。", ephemeral=True
            )
            return

        if digits < 3 or digits > 10:
            await interaction.response.send_message(
                "桁数は3〜10の間で設定してください。", ephemeral=True
            )
            return

        # コンピュータがランダムな指定桁数の数字を選択
        answer = "".join(random.sample("0123456789", digits))
        self.sessions[interaction.guild.id] = {"answer": answer, "attempts": {}}
        ...
        embed.add_field(
            name="ルール",
            value=f"桁と数字が同じならHit。数字は同じで桁が違うならBlow。当ててください。桁数: {digits}桁。",
        )
    ...
    async def on_message(self, message):
        ...
        if (
            len(message.content) != len(session["answer"])
            or not message.content.isdigit()
            or len(set(message.content)) != len(session["answer"])
        ):
            await message.reply(
                f"{len(session['answer'])}桁の重複しない数字を入力してください。"
            )
            return
        ...
        if hits == len(session["answer"]):
            await message.reply(
                f"正解です！数字 {session['answer']} を当てました。ゲーム終了です！"
            )
            del self.sessions[message.guild.id]
        else:
            session["attempts"][message.author.id] = (
                session["attempts"].get(message.author.id, 0) + 1
            )
