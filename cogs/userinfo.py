...
class UserInfo(commands.Cog):
    ...
    @app_commands.command(name="userinfo", description="指定したユーザーの情報を取得します。")
    ...
    async def userinfo(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        embed.add_field(name="アカウント作成日", value=user.created_at.strftime('%Y-%m-%d %H:%M:%S UTC'), inline=False)
        
        # Check if user is a bot
        embed.add_field(name="ボット", value="はい" if user.bot else "いいえ", inline=False)
        ...
        if interaction.guild:
            ...
            if member:
                ...
                embed.add_field(name="ロール", value='\n'.join(roles) if roles else "なし", inline=False)
