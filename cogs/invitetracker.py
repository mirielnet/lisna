...
class InviteTracker(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # 招待を特定
        invites_before = self.server_settings.get(member.guild.id, {}).get('invites', [])
        invites_after = await member.guild.invites()

        inviter = None
        for invite in invites_after:
            before_invite = discord.utils.get(invites_before, code=invite.code)
        ...
        # サーバーの現在の招待リンクを更新
        self.server_settings[member.guild.id] = {'invites': invites_after}
