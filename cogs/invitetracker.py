...
class InviteTracker(commands.Cog):
        ...
        if settings["channel_id"]:
            ...
            if channel:
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"{member.mention}は{inviter_mention}からの招待でした。現在{await self.get_invite_count(member.guild.id, inviter_id)}人招待しています。"
                        if inviter_id
                        else f"{member.mention}の招待者は不明です。"
                    ),
                    color=discord.Color.red(),
                )
                await channel.send(embed=embed)
    ...
    @app_commands.command(
        name="invitetracker-set", description="Invite Trackerの設定を行います。"
    )
    @app_commands.describe(
        is_enabled="機能を有効にするかどうか",
        channel="入出メッセージのチャンネルを選択してください。",
    )
    async def set_invite_tracker(
        self,
        interaction: discord.Interaction,
        is_enabled: bool,
        channel: discord.TextChannel = None,
    ) -> None:
        await self.update_server_settings(
            interaction.guild.id, is_enabled, channel.id if channel else None
        )
        await interaction.response.send_message(f"Invite Tracker設定を更新しました。")

    @app_commands.command(
        name="invitetracker", description="自分の招待数を確認します。"
    )
    async def invite_tracker(self, interaction: discord.Interaction) -> None:
        ...
        invite_count = await self.get_invite_count(
    ...
    @app_commands.command(
        name="invitetracker-server",
        description="サーバーの招待数ランキングを表示します。",
    )
    async def invite_tracker_server(self, interaction: discord.Interaction) -> None:
        ...
        for i in range(0, len(rankings), 10):  # 10位までに制限
            embed = discord.Embed(
                ...
                description="\n".join(
                    [
                        f"{idx + 1}. {self.bot.get_user(row[1]).mention if self.bot.get_user(row[1]) else '不明なユーザー'}: {row[2]}招待"
                        for idx, row in enumerate(rankings[i : i + 10])
                    ]
                ),
    ...
    async def get_server_settings(self, guild_id: int) -> dict:
        result = await db.execute_query(
            "SELECT is_enabled, channel_id FROM invite_tracker_settings WHERE guild_id = $1",
            (guild_id,),
        )
        if result:
            return {
    ...
    async def update_server_settings(
        self, guild_id: int, is_enabled: bool, channel_id: int
    ) -> None:
        await db.execute_query(
            """
    ...
    async def get_inviter(self, guild_id: int, user_id: int) -> int:
        result = await db.execute_query(
            "SELECT inviter_id FROM invite_tracker WHERE guild_id = $1 AND user_id = $2",
            (guild_id, user_id),
        )
    ...
    async def get_invite_count(self, guild_id: int, user_id: int) -> int:
        result = await db.execute_query(
            "SELECT invites FROM invite_tracker WHERE guild_id = $1 AND user_id = $2",
            (guild_id, user_id),
        )
    ...
    async def get_server_ranking(self, guild_id: int) -> list:
        return await db.execute_query(
            "SELECT guild_id, user_id, invites FROM invite_tracker WHERE guild_id = $1 ORDER BY invites DESC",
            (guild_id,),
        )
