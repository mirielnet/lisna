...
class MIQCog(commands.Cog):
    ...
    @app_commands.command(
        name="miq",
        description="DiscordのメッセージリンクまたはメッセージIDから情報を取得して画像を生成します。",
    )
    async def miq(self, interaction: discord.Interaction, message_link_or_id: str):
        ...
        try:
            ...
            await interaction.response.send_message(
                "指定されたメッセージが見つかりませんでした。", ephemeral=True
            )
            return
        ...
        icon_url = (
            target_message.author.avatar.url
            if target_message.author.avatar
            else target_message.author.default_avatar.url
        )

        # GETリクエストを送信
        params = {"name": name, "id": user_id, "content": content, "icon": icon_url}
        ...
        try:
            ...
            await interaction.response.send_message(
                f"リクエストに失敗しました: {e}", ephemeral=True
            )
