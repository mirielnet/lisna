...
class Chunithm(commands.Cog):
    ...
    async def chu_profile(self, interaction: discord.Interaction, user_name: str):
        ...
        if response.status_code == 200:
            ...
            embed.add_field(name="プレイヤー名", value=data['player_name'], inline=False)
            embed.add_field(name="称号", value=data['title'], inline=False)
            embed.add_field(name="称号のレアリティ", value=data['title_rarity'], inline=True)
            embed.add_field(name="レベル", value=data['level'], inline=True)
            embed.add_field(name="レーティング", value=data['rating'], inline=True)
            embed.add_field(name="最大レーティング", value=data['rating_max'], inline=True)
            embed.add_field(name="クラスエンブレム", value=data['classemblem'], inline=True)
            embed.add_field(name="チームに参加", value="はい" if data['is_joined_team'] else "いいえ", inline=True)
